                     ...
                     ecma_string_t *property_name_p) /**< property name */
{
  ...
  do
  {
    ...
    if (unlikely(ecma_is_value_found (value)))
    {
      return value;
    }
                    ...
                    bool is_throw) /**< flag that controls failure handling */
{
  ...
  else
  {
    ...
    if (unlikely(create_new_property
                                                                  ...
                                                                  *   false - list only own properties */
{
                                                        ...
                                                        : NULL)
  {
    ...
    while (prop_iter_p != NULL)
    {
      ...
      for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
      {
            ...
            || ECMA_PROPERTY_GET_TYPE (*property_p) == ECMA_PROPERTY_TYPE_NAMEDACCESSOR))
        {
          ...
          if (unlikely(!(is_enumerable_only && !ecma_is_property_enumerable (*property_p))))
          {
            uint8_t hash = (uint8_t) name_p->hash;
            uint32_t bitmap_row = (uint32_t) (hash / bitmap_row_size);
            uint32_t bitmap_column = (uint32_t) (hash % bitmap_row_size);
    ...
    while (ecma_collection_iterator_next (&iter))
    {
      ...
      else if (unlikely(!is_array_indices_only))
      {
         ...
         i++)
    {
      ...
      if (unlikely((names_hashes_bitmap[bitmap_row] & (1u << bitmap_column)) == 0))
      {
...
{
  if (unlikely(ecma_get_object_type (object_p) == ECMA_OBJECT_TYPE_CLASS))
  {
    ecma_extended_object_t *ext_object_p = (ecma_extended_object_t *) object_p;
