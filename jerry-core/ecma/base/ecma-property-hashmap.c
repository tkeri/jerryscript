...
ecma_property_hashmap_create (ecma_object_t *object_p) /**< object */
{
  ...
  while (prop_iter_p != NULL)
  {
    ...
    for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
    {
      ...
      if (likely(type == ECMA_PROPERTY_TYPE_NAMEDDATA || type == ECMA_PROPERTY_TYPE_NAMEDACCESSOR))
      {
  ...
  while (prop_iter_p != NULL)
  {
    ...
    for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
    {
      ...
      if (likely(mask < LIT_STRING_HASH_LIMIT))
      {
        entry_index &= mask;
      }
      else
                            ...
                            jmem_cpointer_t *property_real_name_cp) /**< [out] property real name */
{
  ...
  while (true)
  {
    ...
    {
      if (likely(!ECMA_PROPERTY_HASHMAP_GET_BIT (bits_p, entry_index)))
      {
#ifndef JERRY_NDEBUG
        JERRY_ASSERT (!property_found);
#endif /* !JERRY_NDEBUG */
