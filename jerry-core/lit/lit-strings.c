                          ...
                          lit_utf8_size_t buf_size) /**< string size */
{
  ...
  while (idx < buf_size)
  {
    ...
    if (likely((c & LIT_UTF8_1_BYTE_MASK) == LIT_UTF8_1_BYTE_MARKER))
    {
                              ...
                              ecma_char_t *code_point) /**< [out] code point */
{
  ...
  if (likely((c & LIT_UTF8_2_BYTE_MASK) == LIT_UTF8_2_BYTE_MARKER))
  {
    bytes_count = 2;
    ret = ((lit_code_point_t) (c & LIT_UTF8_LAST_5_BITS_MASK));
  }
...
{
  if (likely((first_byte & LIT_UTF8_1_BYTE_MASK) == LIT_UTF8_1_BYTE_MARKER))
  {
    return 1;
  }
  else if (likely((first_byte & LIT_UTF8_2_BYTE_MASK) == LIT_UTF8_2_BYTE_MARKER))
  {
    return 2;
  }
  else
...
{
  if (likely(code_unit <= LIT_UTF8_1_BYTE_CODE_POINT_MAX))
  {
